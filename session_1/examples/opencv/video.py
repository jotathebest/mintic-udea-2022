...
Sample shows how VideoCapture class can be used to acquire video.py
frames from a camera of a movie file. Also the sample provides
an example of procedural video.py generation by an object, mimicking
the VideoCapture interface (see Chess class).

'create_capture' is a convenience function for capture creation,
falling back to procedural video.py in case of error.

Usage:
    video.py.py [--shotdir <shot path>] [source0] [source1] ...'

    sourceN is an
     - integer number for camera capture
     - name of video.py file
     - synth:<params> for procedural video.py
...
def create_capture(source = 0, fallback = presets['chess']):
    ...
    if cap is None or not cap.isOpened():
        print('Warning: unable to open video.py source: ', source)
